...
def get_resolved_securities():
    ...
    if cfg("SP500"):
        tickers.update(getSecurities('http://en.wikipedia.org/wiki/List_of_S%26P_500_companies', sectorPosOffset=3, ETF="SPX"))
    if cfg("SP400"):
...
def create_price_history_file(tickers_dict):
    PRICE_DATA_OUTPUT = os.path.join(os.path.dirname(os.path.realpath(__file__)), "data", "price_history.json")
...
# print all the rows in df_securities, row by row
##for index, row in df_securities.iterrows():
